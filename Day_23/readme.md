...
modified\_list=\[i\*\*2 if i>=10 else i for i in numbers]
...
modified\_list=\[i for i in numbers if i%5==0 and i%10==0 ]
...
modified\_list=\[i.upper() for i in words if len(i)>3]
...
modified\_list=\[i for i in scores if i is not None and i >= 50]
...
modified\_list=\[values for sublist in sales for values in sublist if values >=200 ]
